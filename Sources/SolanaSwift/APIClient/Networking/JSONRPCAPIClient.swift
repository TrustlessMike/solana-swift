...
public class JSONRPCAPIClient: SolanaAPIClient {
    ...
    public func getTransaction(transactionSignature: String) async throws -> TransactionInfo {
        try await get(method: "getTransaction", params: [transactionSignature, RequestConfiguration(encoding: "jsonParsed", maxSupportedTransactionVersion: 0)])
    }
    ...
    public func getRecentBlockhash(commitment: Commitment? = nil) async throws -> String {
        let result: Rpc<SOLRPCLatestBlockhashValue> = try await get(
...
// Define the struct for the getLatestBlockhash response value
// This should be placed where it's accessible to JSONRPCAPIClient, 
// e.g., outside the class or as a nested type if that's the pattern.
public struct SOLRPCLatestBlockhashValue: Codable {
